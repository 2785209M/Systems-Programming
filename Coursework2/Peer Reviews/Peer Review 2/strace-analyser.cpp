...
public:
    ...
    void push(std::string line) {
        /* TODO: implement when adding concurrency */
    ...
    bool pop(std::string &out) {
        ...
        if (q_.empty() && closed_) {
            return false; /* closed, no more work */
        }
...
int main(int argc, char *argv[]) {
    ...
    if (argc >= 3) {
        ...
        std::cerr << "Note: Sequential version ignores num_threads.\n";
    }
    ...
    /*producer step: read entire file and push to workQueue*/
    std::string line;
    ...
    /*consumer step: pop from workQueue and update stats*/
    StatsMap stats;
    std::mutex statsMutex;

    std::vector<std::thread> workers; //vector to hold multiple worker threads
    ...
    for (int i = 0; i < numThreads; ++i) {
        workers.emplace_back([&workQueue, &stats, &statsMutex]() {
            std::string line;
            while (workQueue.pop(line)) {
                std::string syscall;
                int result;
                if (parse_line(line, syscall, result)) {
