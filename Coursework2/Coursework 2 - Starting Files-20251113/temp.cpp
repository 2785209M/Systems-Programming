...
public:
    ...
    bool pop(std::string &out) {
        ...
        // Wait while queue is empty, unless closed
        cv_.wait(lock, [&] {
            return !q_.empty() || closed_;
        });

        // If closed and no work, signal end
        if (q_.empty() && closed_) {
            return false;
        }
...
int main(int argc, char *argv[]) {
    ...
    std::vector<StatsMap> allStats(numThreads);

    for (int i = 0; i < numThreads; i++){
        workers.emplace_back([&workqueue, &allStats, i](){
            std::string line; // Create a string on which to write the output of pop
            while (workqueue.pop(line)) { // Loop whilst the queue isn't closed and empty. This is how threads wait for work.
                std::string syscall; // String on which to write the syscall name
                int result; // int to return the result of parsing
                if (parse_line(line, syscall, result)){ // If parsing succeeds ...
                    update_stats(allStats[i], syscall, result); // ... Update the StatsMap, Syscall, and the result values
                }
            }
        });
    }
    ...
    workqueue.close(); // Close the workQueue. Put the threads to sleep.
    ...
    StatsMap finalStats;
    for (const auto &m : allStats){
        for (const auto &p : m){
            const std::string &syscall = p.first;
            const Stats &s = p.second;
            Stats &dst = finalStats[syscall];
            dst.count += s.count;
            dst.fails += s.fails;
        }
    }

    print_stats(finalStats);
    return 0;
}
